...
class AdsController {
	...
	update(req: express.Request, res: express.Response) {
		...
		AdModel.findByIdAndUpdate(adId, updateData, { new: true }).exec(function (err, ad) {
			if (err)
				return res.json({
	...
	delete(req: express.Request, res: express.Response) {
		...
		AdModel.findOneAndRemove({ _id: id })
			.then((ad) => {
				if (ad) {
					res.json({
